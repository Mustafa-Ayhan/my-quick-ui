...
export function mergeAnnotations(nodes) {
    const nonEmpty = (t) => !!t;
    ...
    const examples = uniq([].concat(...nodes.map(n => ensureArray(n.examples)))
        .filter(nonEmpty));
    const _default = join(nodes.map(n => n.default));
    const see = uniq([].concat(...nodes.map(n => ensureArray(n.see)))
        .filter(nonEmpty));
...
export function wrapWhitespace(text) {
    if (!text.includes("\n"))
...
export function stringifyAnnotations(node, { includeComment = false, formatWhitespace = false, } = {}) {
