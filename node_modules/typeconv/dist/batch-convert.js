...
export async function batchConvert(converter, filenames, options) {
    ...
    const { root, newRoot, files } = reRootFiles(filenames, cwd, options.outputDirectory);
    if (verbose) {
    ...
    await map(files, { concurrency }, async ({ in: filename, out: outFilename, rel }) => {
        ...
        if (verbose) {
            ...
            console.error(`${prefix} ${prettyFile(rel, root)} -> ` +
...
export async function batchConvertGlob(converter, globs, options) {
