...
import { getJsonSchemaReader, getJsonSchemaWriter, getOpenApiReader, getOpenApiWriter, } from "../convert-json-schema.js";
import { getGraphQLReader, getGraphQLWriter, } from "../convert-graphql.js";
import { getTypeScriptReader, getTypeScriptWriter, } from "../convert-typescript.js";
import { getCoreTypesReader, getCoreTypesWriter, } from "../convert-core-types.js";
...
const { version } = JSON.parse(await fsPromises.readFile(path.resolve(__dirname, "..", "..", "package.json"), 'utf-8'));
const oppaInstance = oppa({
    version: version,
...
const { verbose, "dry-run": dryRun, hidden, "from-type": fromType, "to-type": toType, shortcut, "output-directory": outputDirectory, "output-extension": outputExtension, "strip-annotations": doStripAnnotations, "merge-objects": mergeObjects, 
// TypeScript
"ts-declaration": tsDeclaration, "ts-disable-lint-header": tsDisableLintHeader, "ts-descriptive-header": tsDescriptiveHeader, "ts-use-unknown": tsUseUnknown, "ts-non-exported": tsNonExported, "ts-namespaces": tsNamespaces, 
// JSON Schema
// GraphQL
...
if (!ensureType(fromType, 'type system identifyer', typeImplementations, printHelp))
    throw new Error();
if (!ensureType(toType, 'type system identifyer', typeImplementations, printHelp))
    throw new Error();
if (!ensureType(tsNonExported, 'ts-non-exported', ['fail', 'ignore', 'include', 'inline', 'include-if-referenced'], printHelp))
    throw new Error();
const mergeObjectsDefault = toType === 'jsc' || toType === 'oapi';
if (!ensureType(tsNamespaces, 'ts-namespaces', ['ignore', 'hoist', 'dot', 'underscore', 'reconstruct-all'], printHelp))
    throw new Error();
const toTsNamespaces = tsNamespaces === 'reconstruct-all' ? 'all' :
    tsNamespaces === 'dot' ? 'dot' :
        tsNamespaces === 'underscore' ? 'underscore' :
            'ignore';
const fromTsNamespaces = tsNamespaces === 'hoist' ? 'hoist' :
    tsNamespaces === 'dot' ? 'join-dot' :
        tsNamespaces === 'underscore' ? 'join-underscore' :
            'ignore';
if (!ensureType(stRefMethod, 'ref-method', ['no-refs', 'provided', 'ref-all', undefined], printHelp))
    throw new Error();
if (!ensureType(stMissingReference, 'missing-ref', ['ignore', 'warn', 'error', undefined], printHelp))
    throw new Error();
const getReader = () => {
    return fromType === 'ts'
        ? getTypeScriptReader({
...
(async () => {
    ...
    const converter = makeConverter(getReader(), getWriter(), {
        ...(!doStripAnnotations ? {} :
    ...
    const result = await batchConvertGlob(converter, globFiles, {
        outputDirectory,
        outputExtension: outputExtension ?? (toType === 'ts' ? '.ts'
            : toType === 'jsc' ? '.json'
    ...
    console.error(chalk.bold(`💡 Converted ${result.types} types in ${result.files} files, ` +
        `in ${sec}s`));
})()
    .catch((err) => {
